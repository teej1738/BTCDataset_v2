...
FIXES vs original:
  - Spot early files use microsecond timestamps (not ms) - auto-detected and normalised
  - Funding rate CSV has 3 columns: calcTime, fundingRate, fundingIntervalHours
    (no mark_price - that was a parsing error)
"""
...
# Microsecond threshold: timestamps > this are in us, not ms
# A ms timestamp for year 2100 = ~4102444800000 (4e12)
# A us timestamp for year 2017 = ~1502928000000000 (1.5e15)
US_THRESHOLD = 1_000_000_000_000_0  # 1e13 - anything above this is microseconds
...
# FIX: early Binance spot archives use microseconds, later ones use milliseconds
# Detect by magnitude and normalise everything to milliseconds
# ==============================================================================

def normalise_timestamps_to_ms(series):
    """
    Convert a series of Binance timestamps to milliseconds.
    Values > US_THRESHOLD are treated as microseconds and divided by 1000.
    Values <= US_THRESHOLD are already milliseconds.
    """
    s = series.copy().astype(np.int64)
    us_mask = s > US_THRESHOLD
    s[us_mask] = s[us_mask] // 1000
    return s
...
KLINE_COLS = [
    "bar_start_ts_raw", "open", "high", "low", "close",
    "volume_base", "bar_end_ts_raw", "volume_quote",
    "trade_count", "taker_buy_base", "taker_buy_quote", "ignore_field",
]

def parse_kline_csv(csv_bytes, instrument_type, price_type):
    df = pd.read_csv(
        io.BytesIO(csv_bytes), header=None,
        names=KLINE_COLS, dtype=str
    )

    # Drop any header rows
    df = df[pd.to_numeric(df["bar_start_ts_raw"], errors="coerce").notna()].copy()
    df = df.reset_index(drop=True)

    # Parse raw timestamps
    df["bar_start_ts_raw"] = pd.to_numeric(df["bar_start_ts_raw"]).astype(np.int64)
    df["bar_end_ts_raw"]   = pd.to_numeric(df["bar_end_ts_raw"]).astype(np.int64)

    # FIX: normalise microseconds -> milliseconds
    df["bar_start_ts_ms"] = normalise_timestamps_to_ms(df["bar_start_ts_raw"])
    df["bar_end_ts_ms"]   = normalise_timestamps_to_ms(df["bar_end_ts_raw"])
    ...
    # Drop raw timestamp and ignore columns
    df = df.drop(columns=["bar_start_ts_raw","bar_end_ts_raw","ignore_field"],
                 errors="ignore")
    return df


# FIX: Correct funding rate CSV columns
# Format: calcTime, fundingRate, fundingIntervalHours  (no mark_price)
FUNDING_COLS = ["fund_calc_time_ms", "fund_interval_hours", "fund_rate"]
...
def parse_funding_csv(csv_bytes):
    ...
    # Drop header row if present (some files have a text header)
    ...
    df["fund_interval_hours"] = pd.to_numeric(df["fund_interval_hours"], errors="coerce").astype(float)
...
def download_monthly(name, url_fn, parse_fn, start, end):
    ...
    log.info(
        f"\n  Result: {len(dfs)} months OK | {len(skipped)} skipped | "
        f"{len(failed)} failed | {cs_ok} checksums verified"
    )
    if skipped[:5]:
...
def qa(df, name):
    ...
    if "fund_rate" in df.columns:
        log.info(f"    Rate range: {df['fund_rate'].min():.6f} -> {df['fund_rate'].max():.6f}")
        log.info(f"    Events:     {len(df):,}")
...
def main():
    log.info("=" * 60)
    log.info("  BTCDataset v2 - Step 1: Download (fixed)")
